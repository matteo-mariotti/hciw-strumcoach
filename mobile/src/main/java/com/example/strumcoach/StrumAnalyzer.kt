...
object StrumAnalyzer {
    ...
    fun analyze(
        ...
        // User-tunable detection sensitivity (Settings screen). 1.0 = defaults above;
        // >1.0 lowers thresholds to catch lighter strums, <1.0 raises them.
        sensitivity: Float = 1.0f
    ): AnalysisResult {
        if (gyroZValues.isEmpty()) return AnalysisResult(0, emptyList(), emptyList(), emptyList())

        val gyroThreshold = GYRO_STRUM_THRESHOLD / sensitivity
        val minGyroMagnitude = MIN_GYRO_MAGNITUDE / sensitivity
        val fluxMinLevel = FLUX_MIN_LEVEL / sensitivity
        val fluxProminence = FLUX_PROMINENCE / sensitivity
        ...
        var detectedStrums = if (!trimmedFlux.isNullOrEmpty()) {
            val (strums, minLevel) = findAudioOnsetStrumsByFlux(
                trimmedGyro, trimmedFlux, trimmedTS, fluxMinLevel, fluxProminence, minGyroMagnitude
            )
            audioOnsetThreshold = minLevel
            strums
        } else {
            findGyroStrums(trimmedGyro, trimmedTS, gyroThreshold)
        }
        ...
        */
        ...
        // 6. Confronto
        val feedbackPrefix = if (maxAbs < minGyroMagnitude) "Weak signal: try moving your wrist with more energy. " else ""
    ...
    private fun findGyroStrums(
        ...
        fun flushSegment() {
            ...
            if (abs(peakValue) < gyroThreshold) return
    ...
    private fun findAudioOnsetStrumsByFlux(
        ...
        timestamps: List<Float>,
        fluxMinLevel: Float,
        fluxProminence: Float,
        minGyroMagnitude: Float
        ...
        for (i in 1 until flux.lastIndex) {
            ...
            if (curr < fluxMinLevel || prominence < fluxProminence) continue
            ...
            if (abs(gyroValue) < minGyroMagnitude) continue
        ...
        return detectedStrums to fluxMinLevel
    }
