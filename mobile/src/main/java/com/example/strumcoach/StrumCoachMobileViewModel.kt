...
class StrumCoachMobileViewModel(application: Application) : AndroidViewModel(application),
    ...
    // Editable from Settings since the dev backend's LAN IP changes whenever the
    // machine hosting it switches networks (see PracticeReminderScheduler/AudioManager
    // comments elsewhere for the same recurring issue). Retrofit/api/repository are
    // rebuilt in place when this changes — no app restart needed, and none of the
    // observeXxx() flows in NetworkRepository are long-lived (each is a one-shot
    // fetch), so there's nothing left running against the old backend to race with.
    var backendBaseUrl: String by mutableStateOf(
        prefs.getString("backend_base_url", null)
            ?: "http://192.168.188.159:8000/" // Local Python FastAPI Backend (10.0.2.2 for Android Emulator)
    )
        private set
    private var retrofit = buildRetrofit(backendBaseUrl)
    private var api = retrofit.create(StrumCoachApi::class.java)
    private var repository: StrumCoachRepository = NetworkRepository(api, backendBaseUrl)
    ...
    fun updateBackendBaseUrl(url: String) {
        ...
        if (normalized == backendBaseUrl) return

        backendBaseUrl = normalized
        prefs.edit().putString("backend_base_url", normalized).apply()
        retrofit = buildRetrofit(normalized)
        api = retrofit.create(StrumCoachApi::class.java)
        repository = NetworkRepository(api, normalized)
        refreshData()
    }
    ...
    var themeMode by mutableStateOf(
        com.example.strumcoach.ui.theme.ThemeMode.valueOf(
            prefs.getString("theme_mode", com.example.strumcoach.ui.theme.ThemeMode.SYSTEM.name)
                ?: com.example.strumcoach.ui.theme.ThemeMode.SYSTEM.name
        )
    )
        private set

    fun updateThemeMode(mode: com.example.strumcoach.ui.theme.ThemeMode) {
        themeMode = mode
        prefs.edit().putString("theme_mode", mode.name).apply()
    }

    var detectionSensitivity by mutableFloatStateOf(prefs.getFloat("detection_sensitivity", 1.0f))
        private set

    fun updateDetectionSensitivity(multiplier: Float) {
        detectionSensitivity = multiplier
        prefs.edit().putFloat("detection_sensitivity", multiplier).apply()
    }

    var hapticAmplitude by mutableIntStateOf(prefs.getInt("haptic_amplitude", 200))
        private set

    fun updateHapticAmplitude(amplitude: Int) {
        hapticAmplitude = amplitude
        prefs.edit().putInt("haptic_amplitude", amplitude).apply()
        sendHapticSettingsToWatch()
    }

    var reminderEnabled by mutableStateOf(prefs.getBoolean("reminder_enabled", false))
        private set

    var reminderTime by mutableStateOf(
        com.example.strumcoach.ui.screens.ReminderTime.valueOf(
            prefs.getString("reminder_time", com.example.strumcoach.ui.screens.ReminderTime.EVENING.name)
                ?: com.example.strumcoach.ui.screens.ReminderTime.EVENING.name
        )
    )
        private set
    ...
    fun updateReminderEnabled(enabled: Boolean) {
        ...
        if (enabled) {
            PracticeReminderScheduler.schedule(getApplication(), reminderTime.hour)
        } else {
            PracticeReminderScheduler.cancel(getApplication())
        }
    }

    fun updateReminderTime(time: com.example.strumcoach.ui.screens.ReminderTime) {
        reminderTime = time
        prefs.edit().putString("reminder_time", time.name).apply()
        if (reminderEnabled) {
            PracticeReminderScheduler.schedule(getApplication(), time.hour)
        }
    }

    // Lets PracticeReminderWorker (which runs outside any ViewModel/Compose lifecycle)
    // know whether today's practice already happened, without it having to re-derive
    // sessionHistory itself.
    private fun updatePracticeReminderState() {
        val today = java.time.Instant.now().atZone(java.time.ZoneId.systemDefault()).toLocalDate().toEpochDay()
        prefs.edit()
            .putLong("last_practice_epoch_day", today)
            .putInt("current_streak_snapshot", currentStreak)
            .apply()
    }

    private fun sendHapticSettingsToWatch() {
        viewModelScope.launch {
            try {
                val request = com.google.android.gms.wearable.PutDataMapRequest.create("/settings").apply {
                    dataMap.putInt("hapticAmplitude", hapticAmplitude)
                }.asPutDataRequest().setUrgent()
                dataClient.putDataItem(request).await()
            } catch (e: Exception) {
                android.util.Log.e("ViewModel", "Failed to push haptic settings to watch", e)
            }
        }
    }
    ...
    // Surfaces the exercise the user is currently weakest at (by their last attempt,
    // not an all-time average) so Dashboard can nudge practice where it matters most,
    // instead of always just resuming whatever was played last.
    val suggestedExercise by androidx.compose.runtime.derivedStateOf {
        val lastAttemptPerExercise = sessionHistory
            .filter { !it.isReference && it.exerciseId.isNotEmpty() }
            .groupBy { it.exerciseId }
            .mapValues { (_, sessions) -> sessions.maxByOrNull { it.id }!! }

        (exercises + songs)
            .filter { it.hasReference }
            .mapNotNull { ex -> lastAttemptPerExercise[ex.id]?.let { ex to it.accuracy } }
            .filter { (_, accuracy) -> accuracy < 70 }
            .minByOrNull { (_, accuracy) -> accuracy }
    }

    var currentSessionReport by mutableStateOf<SessionStats?>(null)
        private set

    // Includes the just-finished session even before it round-trips through
    // the backend and reappears in sessionHistory via observeSessions()/refreshData().
    private val effectiveSessions by androidx.compose.runtime.derivedStateOf {
        val report = currentSessionReport
        if (report != null && sessionHistory.none { it.id == report.id }) {
            sessionHistory + report
        } else {
            sessionHistory
        }
    }

    val gamificationXp by androidx.compose.runtime.derivedStateOf {
        com.example.strumcoach.gamification.Gamification.calculateXp(effectiveSessions)
    }

    val gamificationLevel by androidx.compose.runtime.derivedStateOf {
        com.example.strumcoach.gamification.Gamification.calculateLevel(gamificationXp)
    }

    val currentStreak by androidx.compose.runtime.derivedStateOf {
        com.example.strumcoach.gamification.Gamification.calculateStreak(effectiveSessions)
    }

    val unlockedBadges by androidx.compose.runtime.derivedStateOf {
        com.example.strumcoach.gamification.Gamification.unlockedBadges(effectiveSessions)
    }

    private var seenBadgeIds by mutableStateOf(
        prefs.getStringSet("seen_badge_ids", emptySet())?.toSet() ?: emptySet()
    )

    val newlyUnlockedBadges by androidx.compose.runtime.derivedStateOf {
        unlockedBadges.filter { it.id !in seenBadgeIds }
    }

    fun markBadgesSeen(badges: List<com.example.strumcoach.gamification.Badge>) {
        if (badges.isEmpty()) return
        seenBadgeIds = seenBadgeIds + badges.map { it.id }
        prefs.edit().putStringSet("seen_badge_ids", seenBadgeIds).apply()
    }
    ...
    override fun onCapabilityChanged(capabilityInfo: com.google.android.gms.wearable.CapabilityInfo) {
        val wasConnected = isWatchConnected
        isWatchConnected = capabilityInfo.nodes.isNotEmpty()
        if (isWatchConnected && !wasConnected) {
            sendHapticSettingsToWatch()
        }
    }
    ...
    private fun analyzeSensorData(bytes: ByteArray) {
        ...
        // Save session
        if (!isReferenceAtStart) {
            updatePracticeReminderState()
        }
        viewModelScope.launch {
    ...
    // Reference/session audio URLs are persisted on the backend with whatever
    // host was current at upload time. Since the dev backend's LAN IP changes
    // between sessions (see backendBaseUrl above), a URL baked in under an old
    // IP is unreachable even though the file itself is still served fine by
    // the current backend. Re-basing onto the current host on every playback
    // keeps old references playable without a DB migration.
    private fun resolveAudioUrl(url: String): String {
        val uploadsIndex = url.indexOf("/uploads/")
        return if (uploadsIndex >= 0) "${backendBaseUrl.trimEnd('/')}${url.substring(uploadsIndex)}" else url
    }
    ...
    fun togglePlayback(url: String) {
        if (currentlyPlayingUrl == url) {
            ...
            playbackProgress = 0f
            audioManager.playAudio(resolveAudioUrl(url), onComplete = {
