...
class StrumCoachViewModel(application: Application) : AndroidViewModel(application),
    ...
    // Haptic amplitude (0-255) pushed from the phone's Settings screen; 200 is a
    // reasonable default until the first /settings DataItem arrives.
    var hapticAmplitude by mutableIntStateOf(200)
        private set
    ...
    override fun onDataChanged(dataEvents: com.google.android.gms.wearable.DataEventBuffer) {
        for (event in dataEvents) {
            ...
            when (event.dataItem.uri.path) {
                ...
                "/settings" -> {
                    val dataMap = DataMapItem.fromDataItem(event.dataItem).dataMap
                    hapticAmplitude = dataMap.getInt("hapticAmplitude", hapticAmplitude)
                }
            }
        }
    }
